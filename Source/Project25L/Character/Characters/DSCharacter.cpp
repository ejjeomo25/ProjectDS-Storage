...
float ADSCharacter::GetInputThreshold(FGameplayTag& SkillTag)
{
	...
	EDataTableType DataTableType = SkillControlComponent->GetSkillDataTableTypeByCharacter(CurrentCharacterType, -1);
...
void ADSCharacter::InitializeStat()
{
	...
	{
		ECharacterType CharacterType = LocalPlayerState->GetCharacterType();
...
void ADSCharacter::BindWidget(UDSStatComponent* InStatComponent, ACharacter* Character, UDSMainWidget* MainWidget, int32 PlayerIndex)
{
	...
	if (IsValid(Character))
	{
		...
		if (IsValid(LocalPlayerState))
		{
			CurrentCharacterType = LocalPlayerState->GetCharacterType();

			DSEVENT_DELEGATE_BIND(LocalPlayerState->OnCharacterTypeChanged, PlayerInfo, &UDSPlayerInfo::InitPlayerSetting);
			
			DSEVENT_DELEGATE_INVOKE(LocalPlayerState->OnCharacterTypeChanged, CurrentCharacterType, FText::FromString(LocalPlayerState->GetPlayerName()));
		}
	}
}
