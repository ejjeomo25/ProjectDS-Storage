...
float ADSCharacterBase::TakeFinalDamage(float DamageAmount, const FDSDamageEvent& NewDamageEvent, AController* EventInstigator, AActor* DamageCauser)
{
	...
	{
		if (StatComponentPtr->GetClass()->ImplementsInterface(UDamageableInterface::StaticClass()))
		{
			IDamageableInterface::Execute_ReceiveDamage(StatComponentPtr, DamageAmount, NewDamageEvent.DamageType, NewDamageEvent.ElementType, this);
		}
	}
