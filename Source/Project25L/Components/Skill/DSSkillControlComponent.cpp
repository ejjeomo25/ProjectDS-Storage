...
// ---------------------------------------------------------- Skill Activate Functions ----------------------------------------------------------
void UDSSkillControlComponent::ActivatePrimarySKill()
{
...
FDSSkillSpec* UDSSkillControlComponent::FindSkillSpecFromType(ESkillType InType) const
{
	if(InType == ESkillType::None)
	{
		return nullptr;
	}
...
void UDSSkillControlComponent::AddReplicatedInstancedSkill(UDSSkillBase* Skill)
{
	...
	if (!IsValid(Skill))
	{
		UE_LOG(LogTemp, Error, TEXT("[SkillRep] Invalid skill passed."));
		return;
	}

	GetReplicatedInstancedSkills_Mutable().Add(Skill);

	UE_LOG(LogTemp, Warning, TEXT("[SkillRep] Added %s to ReplicatedInstancedSkills (via Mutable Accessor)."),
		*GetNameSafe(Skill));
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("[SkillRep] Skipped AddReplicatedSubObject: ReadyForReplication=%s, UsingSubObjectList=%s"),
			IsReadyForReplication() ? TEXT("true") : TEXT("false"),
...
void UDSSkillControlComponent::InitSkillActorInfo(AActor* InOwnerActor, AActor* InAvatarActor)
{
	...
	OnSkillPressedEvents.Add(ESkillType::PrimarySkill, FOnSkillPressed());
	DSEVENT_DELEGATE_BIND(OnSkillPressedEvents[ESkillType::PrimarySkill], this, &ThisClass::ActivatePrimarySKill);

	OnSkillPressedEvents.Add(ESkillType::SecondarySkill, FOnSkillPressed());
