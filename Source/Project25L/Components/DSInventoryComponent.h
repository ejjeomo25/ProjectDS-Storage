...
public:
	...
	*/
	...
	bool StoreItems(ADSItemActor* ItemActor, const FDSItemInfo& ItemData);

	// 키를 사용해서 아이템 사용을 눌렀을 때 실제로 사용 한다.
	// 현재 아이템 타입이 있는지 없는지를 검사를 수행한다.
	void UseItem(int32 ItemIdx);
...
public:
	...
	FORCEINLINE const FPersonalInventoryInfo& GetPersonalInventoryInfo() const{ return PersonalInventoryInfo;}
	FORCEINLINE const bool GetIsDirty() const{ return IsDirty;}
	FORCEINLINE const TMap<FIntPoint, FDSItemInfo> GetAllItems() const { return AllItems; }


	bool IsRoomAvailable(const FDSItemInfo& ItemInfo, int32 TopLeftIndex);
	static FIntPoint IndextoTile(int32 Index, int32 Columns); // static
	static int32 TiletoIndex(FIntPoint DSTile, int32 Columns); // static
	bool GetEmptyTileIndex(const FDSItemInfo& ItemInfo, int32 TopLeftIndex);
	bool IsItemAtIndex(int32 Index);
	void AddItemAt(const FDSItemInfo& ItemInfo, int32 TopLeftIndex);
...
protected:
	...
	void ServerRPC_StoreItems(ADSItemActor* ItemActor, const FDSItemInfo& ItemData, int32 ClientItemCount);

	// 클라이언트와 서버의 아이템 개수가 다를 경우, 롤백한다.
	UFUNCTION(Client, Reliable)
	void ClientRPC_RollbackItems(bool bIsPlus,FDSItemInfo ItemData);
...
// UI
protected:
	UPROPERTY(EditAnywhere, Category = "DSSettings | PersonalInventoryInfo")
	FPersonalInventoryInfo PersonalInventoryInfo;
