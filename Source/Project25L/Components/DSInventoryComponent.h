...
public:
	...
	FOnInventorySlotUpdated OnItemInventorySlotUpdated;

	DECLARE_MULTICAST_DELEGATE_ThreeParams(FOnInventorySlotUpdated, const FDSItemInfo& ItemInfo, FIntPoint TopLeftIndex, bool bRemoved);
	FOnInventorySlotUpdated OnVehicleInventorySlotUpdated;

	DECLARE_MULTICAST_DELEGATE_OneParam(FOnInventoryFull, EAlertStatus AlertStatus);
	FOnInventoryFull OnInventoryFull;

	FORCEINLINE const TMap<EInventoryCategory, FPersonalInventoryInfo> GetPersonalInventoryInfo() const { return InventoryInfoMap; }
	FORCEINLINE const TMap<EInventoryCategory, TArray<FDSItemInfo>> GetPersonalInventory() const { return InventoryMap; }
...
public:
	...
	TArray<FDSItemInfo>* GetInventoryArray(EInventoryCategory Category) { return InventoryMap.Find(Category); }
	FPersonalInventoryInfo* GetInventoryInfo(EInventoryCategory Category) { return InventoryInfoMap.Find(Category); }
...
protected:
	...
	TMap<EInventoryCategory, TArray<FDSItemInfo>> InventoryMap;

	UPROPERTY(Transient)
	TMap<EInventoryCategory, FPersonalInventoryInfo> InventoryInfoMap;
