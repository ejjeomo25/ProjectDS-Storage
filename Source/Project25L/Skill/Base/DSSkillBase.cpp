...
//*****************코드 리뷰 : 필요없는 로그, 주석 다 삭제 바람 IsValid 체크. ****************************//
...
void UDSSkillBase::OnAddSkill(const FDSSkillActorInfo* ActorInfo, const FDSSkillSpec& Spec)
{
	...
	if (ActorInfo)
	{
		SetCurrentActorInfo(Spec.Handle, Spec.SkillType, ActorInfo);
	}
	OnSkillInitialized();
}
...
void UDSSkillBase::SetCurrentActorInfo(const FDSSkillSpecHandle Handle, const ESkillType InType,  const FDSSkillActorInfo* ActorInfo) 
{
	if (IsInstantiated())
	{
