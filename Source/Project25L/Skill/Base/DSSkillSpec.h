...
struct PROJECT25L_API FDSSkillSpec : public FFastArraySerializerItem
{
	...
	/** 기본 생성자 */
		FDSSkillSpec()
		: Skill(nullptr), SkillType(ESkillType::None), InputID(INDEX_NONE), SourceObject(nullptr), ActiveCount(0), InputPressed(false), RemoveAfterActivation(false), PendingRemove(false), bActivateOnce(false)
	{ }

	/** 스킬 클래스를 이용한 생성자 */
	FDSSkillSpec(TSubclassOf<UDSSkillBase> InSkillClass, ESkillType InSkillType, int32 InInputID = INDEX_NONE, UObject* InSourceObject = nullptr);

	/** 스킬 인스턴스를 직접 넘기는 생성자 */
	FDSSkillSpec(UDSSkillBase* InSkill, ESkillType InSkillType, int32 InInputID = INDEX_NONE, UObject* InSourceObject = nullptr);
